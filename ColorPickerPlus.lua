...
-- Safe call to Blizzard ColorPickerFrame.func()
local function CallColorPickerFunc()
    if ColorPickerFrame and ColorPickerFrame.func then
...
function MOD:CleanUpColorPickerFrame()
	...
	if isClassic or isWrath then
		...
		if ColorPickerFrame.TitleText then
			ColorPickerFrame.TitleText:Hide()
		end
	
		local h = CreateFrame("Frame", "ColorPPHeaderTitle", ColorPickerFrame)
		h:SetSize(200, 20)
		-- Anchor it to the top of the color picker **content area**, not the absolute frame top
		h:SetPoint("TOP", ColorPickerFrame, "TOP", 0, -4)

		local t = h:CreateFontString(nil, "OVERLAY", "GameFontNormal")
		t:SetText("Color Picker Plus")
		-- Anchor to the top center of the header frame
		t:SetPoint("TOP", h, "TOP", 0, 0)
